...
function severityHeading(severity: Severity): string {
  switch (severity) {
    case "critical": {
      return "Critical — Must fix now";
    }
    case "performance": {
      return "Performance — Causes slowdowns";
    }
    case "best-practice": {
      return "Best Practices — Recommended improvements";
    }
    case "info": {
...
export async function exportToMarkdown(
  ...
  if (result.securityAudit.summary.total > 0) {
    ...
    lines.push(`## 🔒 Security Vulnerabilities (${parts.join(", ")})`);
    lines.push("");
    ...
    lines.push("");
    lines.push("> 💊 Run `bun update` or `bun update --latest` to fix vulnerable packages.");
    lines.push("");
  }
  ...
  if (result.stats.largeFiles.length > 0) {
    ...
    for (const file of result.stats.largeFiles) {
      lines.push(`- \`${file.file}\` — ${file.lines} lines`);
    }
    lines.push("");
  }
